...
async function loadPortfolio() {
  ...
  d.by_asset_class.forEach(ac => {
    const members = ac.asset_ids.join(", ");
    rows += '<tr><td>' + members + '</td><td>' + badge(ac.bucket) + '</td><td>' + money(ac.value) + '</td><td>' + fmt(ac.pct*100,1) + '%</td></tr>';
  });
...
async function loadWMP() {
  ...
  const rows = await Promise.all(wmps.map(async code => {
    try {
      ...
      const pr = await get('/api/market/prices?assets=' + encodeURIComponent(code) + '&start=' + start + '&end=' + today);
