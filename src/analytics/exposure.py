...
class ExposureAnalyzer:
    ...
    def classify(self, product_type: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        if metadata:
            ...
            if pt == "bank_wmp" and (
                ...
                or metadata.get("asset_class") == "fixed_income"
            ):
                return "fixed_income"
    ...
    def analyze(
        ...
        product_registry: Any,
        total_value: float,
        cash_breakdown: Optional[Dict[str, float]] = None,
    ) -> ExposureReport:
        """
        ...
        Args:
            ...
            cash_breakdown: Optional dict of currency -> cash value.  Cash is
                classified as ``"cash"`` and aggregated separately so it does
                not pollute the position ``asset_ids`` list.
        """
        asset_class_groups = {}
        currency_groups = {}
        ...
        # ── Merge cash breakdown (separate from positions) ──────────────
        if cash_breakdown:
            cash_total = sum(cash_breakdown.values())
            if "cash" not in asset_class_groups:
                asset_class_groups["cash"] = {"value": 0.0, "assets": []}
            asset_class_groups["cash"]["value"] += cash_total

            for curr, amount in cash_breakdown.items():
                if curr not in currency_groups:
                    currency_groups[curr] = {"value": 0.0, "assets": []}
                currency_groups[curr]["value"] += amount
                # Cash currency items do NOT get asset_ids (they're not symbols)
