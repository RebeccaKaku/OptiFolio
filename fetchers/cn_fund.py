...
# 如果你已经有了真实的 BaseFetcher 导入路径，请替换下面这两行
...
class CnFundFetcher(BaseFetcher):
    ...
    def fetch(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        """主抓取入口：负责智能路由"""
        print(f"    [采购-CN] 准备抓取: {symbol} | 日期: {start_date} -> {end_date}")
        ...
        if '货币' in f_type:
            ...
            return self._fetch_money_fund(symbol, start_date, end_date)
            
        elif 'ETF' in f_name.upper() and '联接' not in f_name:
            print(f"    [路由判定] 📈 场内ETF -> {f_name} ({f_type})")
            return self._fetch_etf(symbol, start_date, end_date)
            
        else:
            print(f"    [路由判定] 🛒 场外公募 -> {f_name} ({f_type})")
            return self._fetch_open_fund(symbol, start_date, end_date)

    # ---------------------------------------------------------
    # 具体的抓取分支逻辑 (已做异常保护和格式统一)
    # ---------------------------------------------------------
    ...
    def _fetch_etf(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            print(f"    [Error] ETF {symbol} 抓取异常: {e}")
            return pd.DataFrame()
    ...
    def _fetch_open_fund(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        try:
            ...
            print(f"    [Error] 场外公募 {symbol} 抓取异常: {e}")
            return pd.DataFrame()
    ...
    def _fetch_money_fund(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        try:
            ...
            return sliced_df[['Open', 'High', 'Low', 'Close', 'Volume']]
        except Exception as e:
            print(f"    [Error] 货币基金 {symbol} 抓取异常: {e}")
            return pd.DataFrame()