...
def _canonicalize_symbol(symbol: str, asset_type: str | None = None) -> str:
    """Convert a display symbol to its canonical instrument ID."""
    if not symbol or not isinstance(symbol, str):
        raise ValueError("symbol cannot be empty")
    return normalize_instrument_id(symbol.strip(), asset_type=asset_type)
...
class DataProvider:
    ...
    def prices(self, symbol: str, start: str = None, end: str = None,
               mode: str = "fast", asset_type: str | None = None) -> Optional[pd.Series]:
        """Close price series for one asset."""
        if not symbol:
            return None
        canonical = _canonicalize_symbol(symbol, asset_type=asset_type)
        if mode == "live":
            self._trigger_refresh([canonical])
        panel = self._store.get_prices([canonical], start=start, end=end)
        if panel.empty or canonical not in panel.columns:
            return None
        return panel[canonical].dropna().rename(symbol)

    def ohlcv(self, symbol: str, start: str = None, end: str = None,
              mode: str = "fast", asset_type: str | None = None) -> pd.DataFrame:
    ...
    def panel(self, symbols: List[str], start: str = None, end: str = None,
        ...
        canonicals = [_canonicalize_symbol(s, asset_type=asset_type) for s in symbols]
        if mode == "live":
            self._trigger_refresh(canonicals)
        panel = self._store.get_prices(canonicals, start=start, end=end)
        if panel.empty:
            return panel
    ...
    def returns(self, symbol: str, start: str = None, end: str = None,
                frequency: str = "D", asset_type: str | None = None) -> pd.Series:
        """Percentage returns for one asset."""
        prices = self.prices(symbol, start=start, end=end, asset_type=asset_type)
    ...
    def metrics(self, symbol: str, metric: str | List[str] = "all",
                ...
                asset_type: str | None = None) -> Dict[str, float]:
    ...
    _RATE_OBSERVATION_MAP: dict[str, str] = {
        "1y_cn":  "rate.cn.shibor.1y",
        "5y_cn":  "rate.cn.shibor.1y",   # no CGB yield stored yet - closest proxy
        "10y_cn": "rate.cn.shibor.1y",   # no CGB yield stored yet - closest proxy
        "1y_us":  "rate.us.sofr.on",
    }
    ...
    def _rate_series_id(rate_id: str) -> str:
        ...
        if rate_id in DataProvider._RATE_OBSERVATION_MAP:
            return DataProvider._RATE_OBSERVATION_MAP[rate_id]
        # Allow callers to pass canonical IDs or old RATE_* IDs directly.
        try:
            return normalize_instrument_id(rate_id, asset_type="rate")
        except Exception as exc:
            raise ValueError(f"Unrecognized rate_id: {rate_id}") from exc
    ...
    def rate(
        ...
        2. If mode == "live" and the series is missing, fetch and cache it,
           then retry from the store.
        3. Repo exists but no observation for this series - return missing_observation.
        """
        ...
        # Repo available but series missing - try live fetch if requested.
        if repo is not None and hasattr(repo, "latest_observation") and mode == "live":
            try:
                from findata.rates import sync_rate_series

                sync_rate_series(series_id, repo=repo)
            except Exception:
                pass
            result = _lookup()
            if result is not None:
        ...
        return {
            ...
            "warning": (
                ...
                "Use mode='live' to fetch or run tools/sync_macro_rates.py."
            ),
        }
    ...
    def fx_rate(
        ...
        """Get FX rate from store; fetch live and cache if mode == "live"."""
        from findata.fx import FindataFxProvider
        from optifolio_contracts.fx import FxRateError
        ...
        if mode == "live" and repo is not None:
            try:
                from findata.fx_sync import sync_fx_rate

                sync_fx_rate(from_currency, to_currency, repo=repo)
            except Exception:
                pass
            rate = fx.get_rate(from_currency, to_currency, as_of=as_of_date)
            if rate is not None and rate > 0:
    ...
    def export(self, symbol: str, start: str = None, end: str = None,
               format: str = "csv", asset_type: str | None = None) -> str:
