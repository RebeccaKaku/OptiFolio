...
class Orchestrator:
    ...
    def _last_update(self, asset_id: str) -> Optional[datetime]:
        ...
        *asset_id*, or ``None`` if unknown."""
        try:
            report = self._store.missing_report([asset_id])
            if report is not None and not report.empty:
                ...
                if report["observations"].sum() > 0:
                    ...
                    prices = self._store.get_prices([asset_id])
                    if prices is not None and not prices.empty and asset_id in prices.columns:
                        valid = prices[asset_id].dropna()
                        if not valid.empty:
    ...
    def _determine_start_date(self, asset_id: str) -> str:
        ...
        """
        try:
            existing = self._store.get_prices([asset_id])
